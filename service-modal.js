(() => {
  const services = {
    ...
    canvas:{accent:'#ffd400',title:'РИСОВАНИЕ НА ХОЛСТЕ',image:'assets/risovanie.png',intro:'Создаём картины и авторские работы на холсте по вашему эскизу, фотографии, референсу или собственной идее.',items:['Картины по вашему сюжету или референсу','Портреты и декоративные композиции','Выбор формата, палитры и стилистики','Работы под интерьер или подарок'],fits:'Дом, офис, подарок, интерьерные композиции, памятные и авторские работы.',process:'Обсуждаем идею → согласовываем эскиз и стиль → выполняем работу → показываем результат → подготавливаем к передаче.'},
  ...
  const serviceKey = el => {
    ...
    if(title.includes('холст')) return 'canvas';
  ...
  css.textContent = `
    ...
    .service-fullscreen__visual:after{content:'';position:absolute;inset:auto 0 0;height:45%;background:linear-gradient(transparent,rgba(3,4,8,.96))!important;pointer-events:none}
  ...
  document.addEventListener('click',e=>{
    if(e.target.closest('.service-fullscreen'))return;
    const button=e.target.closest('[data-open-service]');
    if(button){const card=button.closest('.service');if(card){e.preventDefault();e.stopPropagation();open(card)}return;}
    const card=e.target.closest('.service');
    if(card){e.preventDefault();e.stopPropagation();open(card)}
  },true);
