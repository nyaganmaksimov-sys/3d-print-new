...
(() => {
  ...
  // Подтверждённые данные добавляем только там, где удалось сопоставить конкретную страницу.
  const verified = {
    'Great Wall Wingle 5 — крепление двери багажника': {
      url: 'https://3dtoday.ru/3d-models/khobbi/avtomobili/great-wall-wingle-5-kreplenie-dveri-bagazhnika',
      author: 'не удалось надёжно извлечь из страницы',
      license: 'проверить на странице модели'
    },
    'Geely Coolray A3 — органайзер в подлокотник': {
      url: 'https://3dtoday.ru/blogs/v0lk',
      author: 'v0lk',
      license: 'проверить на странице модели'
    }
  };
  ...
  grid.innerHTML = models.map(([name,cat,meta],i)=>{
    const v=verified[name];
    const author=v?.author || 'указать после проверки';
    const license=v?.license || 'проверить на странице модели';
    const url=v?.url || sourceUrl;
    return `<article class="product" data-cat="${cat}"><div class="product-image placeholder model-placeholder ${modelClass(name)}"><div class="model-stage"><div class="model-object"></div><div class="model-gridline"></div><span class="model-axis">X&nbsp;&nbsp;&nbsp;Y&nbsp;&nbsp;&nbsp;Z</span></div><div class="model-label"><b>${cat==='auto'?'AUTO':'TECH'}</b><small>3D PREVIEW ${String(i+1).padStart(2,'0')}</small></div></div><div class="product-info"><span>${cat==='auto'?'Авто':'Электроника'} · ${meta}</span><h3>${name}</h3><div class="model-credit"><small>Источник: 3DToday</small><small>Автор: ${author}</small><small>Лицензия: ${license}</small></div><div class="model-actions"><a href="${url}" target="_blank" rel="noopener noreferrer">${v?'Страница модели →':'3DToday →'}</a><b>Рассчитать стоимость →</b></div></div></article>`;
  }).join('');
